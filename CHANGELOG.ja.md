...
- **一時停止 / 再開**（ネイティブ HLS）:進捗を保持したまま一時停止し、途中から再開できます
- **途中で保存**:途中で止まった・最後まで待ちたくないときに、ダウンロード済みの部分を再生可能な動画として結合
- ダウンロード失敗の診断に役立つ匿名の使用統計(設定でオフにできます)

### 改善

- より多くのストリーミングサイトでの検出・互換性を改善(MacCMS 系プレーヤーはページのソースから直接 m3u8 を解析)

## [0.3.6] - 2026-06-10

### 追加

- **字幕**:字幕の言語を個別に選択可能。「all」は自動翻訳ではなくオリジナル字幕のみ取得し、スロットリングを回避
- **yt-dlp の並列フラグメント**(既定 4)で長尺動画が大幅に高速化
- タスク一覧:**完了したものをクリア**

### 変更

- **無料版の改定**:7 日間の無料トライアル + 1080p 上限(旧来の 10 分 / 100 MB 制限を置き換え)。Pro で 4K/8K・クリップ抽出・アプリ内再生を解放

### 修正

- クリップ抽出:長さが全体表示になる、進捗が 0% で止まる、字幕が英語のみになる問題
- クリップ抽出からの Pro ダウンロードが無料版制限として扱われる問題
